...
export class CampanhaService {
  ...
  detalharCampanha(id: string): Observable<DetalharCampanhaResponse> {
    return this.http.get<DetalharCampanhaResponse>(this.config.apiUrl + '/campanha/detalhar/' + id,
      ...
      catchError(UtilService.handleError<DetalharCampanhaResponse>({
  ...
  carregarAcompanhamentos(id: string): Observable<ListarAcompanhamentosResponse> {
    return this.http.get<ListarAcompanhamentosResponse>(this.config.apiUrl + '/campanha/listarDataAcompanhamentos/' + id,
      this.auth.getHttpOptions())
      .pipe(
      tap(
        response => {
          if (response.type === 'error') {
            ...
            for (let i = 0; i < response.datas.length; i++) {
              const date = new Date(response.datas[i]['data']);
              date.setTime(date.getTime() + 3600000 * this.config.hourAdjust);
              response.datas[i]['data'] = date;
            }
            return response;
          }
        }
      ),
      catchError(UtilService.handleError<ListarAcompanhamentosResponse>({
        type: 'error', msg: 'Erro de conexão com a API.',
        datas: []
      }))
      );
...
class ListarAcompanhamentosResponse {
  type: string;
  msg: string;
  datas: Date[];
}
